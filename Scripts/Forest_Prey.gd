...
func _ready():
	...
	if creature_manager != null:
		...
		creature_manager.add_forest_creature(self)
		creature_manager.add_forest_prey(self)
	...
	if is_female:
		...
		var material = load("res://Assets/CreatureShaders/Forest_Prey.tres").duplicate()  # Load the material and duplicate it
		material.albedo_color = Color(0.647, 0.373, 0.275)  # Set the new color
	
		$Main.set_surface_override_material(0,material) 
		$Leg1.set_surface_override_material(0,material) 
...
func _on_sensory_area_entered(area):
	...
	if area.is_in_group("forest_pred") and !is_child:
		enemy = area
	...
	if is_female and !is_child and reproduction == 1:
		if area.is_in_group("forest_prey") and !has_mated and !area.get_parent().is_female and partners != 2 and !area.get_parent().is_child and area.get_parent().reproduction == 1:
...
func _on_self_area_entered(area):
	if area.is_in_group("forest_pred") and area.get_parent()._hungry():
	...
	if area.is_in_group("forest_prey") and partners == 2 and area == mating_partner:
...
func create_child(size,initial_speed,accel,hunger,meta,mother_area,speed_counter, speed):
	
	# Create a new instance of the same creature as a child
	var child = load("res://Scenes/Prey/Forest_Prey.tscn").instantiate()
