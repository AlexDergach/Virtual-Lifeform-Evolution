...
func _ready():
	...
	if creature_manager != null:
		...
		creature_manager.add_stone_pred(self)
...
func _process(delta):
	...
	if hunger == 0:
		...
		creature_manager.remove_stone_pred(self)
...
func _on_sensory_area_entered(area):
	...
	if is_female and !is_child and reproduction == 1:
		if area.is_in_group("stone_pred") and !has_mated and !area.get_parent().is_female and partners != 2 and !area.get_parent().is_child and area.get_parent().reproduction == 1:
...
func _on_self_area_entered(area):
	...
	if area.is_in_group("stone_pred") and partners == 2 and area == mating_partner:
