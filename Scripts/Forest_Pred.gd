...
#@onready var navigation_region: NavigationRegion3D = get_node("/root/Terrian/NavigationRegion3D") #<- For Test Map
...
func _ready():
	...
	if creature_manager != null:
		...
		creature_manager.add_forest_pred(self)
	...
	if is_child:
		...
		creature_manager.forest_pred_gen_score(a, self.generation)	
	...
	creature_manager.add_forest_pred_gender(is_female)
...
func _process(delta):
	...
	if hunger == 0:
		...
		creature_manager.remove_forest_pred(self)
...
func _on_sensory_area_entered(area):
	...
	if area.is_in_group("forest_prey") && _hungry():
	...
	if is_female and !is_child and reproduction == 1:
		if area.is_in_group("forest_pred") and !has_mated and !area.get_parent().is_female and partners != 2 and !area.get_parent().is_child and area.get_parent().reproduction == 1:
...
func _on_self_area_entered(area):
	...
	if area.is_in_group("forest_prey") and _hungry():
		food_target = false
		hunger += 1
		print("Pred: Food ate")
		prey = null
	...
	if area.is_in_group("forest_pred") and partners == 2 and area == mating_partner:
		#print("Mate: ", mate_chosen, " Touched Sending Repo State")
		$StateChart.send_event("repo")

#If Prey Leaves The Sensory Area

func _on_sensory_area_exited(area):
	if area.is_in_group("forest_prey"):
		#print("Prey left")
		$StateChart.send_event("prey_exited")
