...
func _ready():
	...
	if creature_manager != null:
		...
		creature_manager.add_fire_pred(self)
	...
	if is_female:
		...
		material.albedo_color = Color(0.67, 0.2, 0.35)  # Set the new color
		...
		$Tail2.set_surface_override_material(0,material) 
...
func _process(delta):
	...
	if hunger == 0:
		...
		creature_manager.remove_fire_pred(self)
...
func _on_sensory_area_entered(area):
	...
	if area.is_in_group("fire_prey") && _hungry():
	...
	if is_female and !is_child and reproduction == 1:
		if area.is_in_group("fire_pred") and !has_mated and !area.get_parent().is_female and partners != 2 and !area.get_parent().is_child and area.get_parent().reproduction == 1:
...
func _on_self_area_entered(area):
	...
	if area.is_in_group("fire_prey") and _hungry():
		food_target = false
		hunger += 1
		#print("Pred: Food ate")
		prey = null
	...
	if area.is_in_group("fire_pred") and partners == 2 and area == mating_partner:
