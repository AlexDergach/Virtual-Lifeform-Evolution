...
func _on_sensory_area_entered(area):
	...
	if is_female and !is_child and reproduction == 1:
		if area.is_in_group("desert_pred") and !has_mated and !area.get_parent().is_female and partners != 2 and !area.get_parent().is_child and area.get_parent().reproduction == 1:
	...
	if !is_child:
		if partners == 2:
			...
			if food_target == false:
				...
				if global_position.distance_to(nav.target_position) < 0.1 or global_position.distance_to(nav.target_position) == 0: 
